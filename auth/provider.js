...
class AuthProvider {
  ...
  login(options = {}) {
    return async (req, res, next) => {
      ...
      console.log('GENERATING MS LOGIN REDIRECTION URL');
  ...
  handleRedirect(options = {}) {
    return async (req, res, next) => {
      if (!req.body || !req.body.state) {
        return next(new BadRequestError('Error: response not found'));
      }
      if (!req.session) throw new BadRequestError('Invalid cookie');
      ...
      try {
        ...
        // REDIRECT TO FRONTEND ALONG WITH COOKIE
  ...
  logout(options = {}) {
    return (req, res, next) => {
      try {
        ...
        req.session.destroy(() => {
          res.clearCookie('cid.session', { path: '/' });
          res.clearCookie('cid.state', { path: '/' });
          res.send(successResponse({ logoutUri }));
        });
      } catch (error) {
        console.log(error);
        next();
  ...
  redirectToAuthCodeUrl(
    ...
    return async (req, res, next) => {
      ...
      console.log('MS AUTH REDIRECTION URL GENERATED');
      try {
